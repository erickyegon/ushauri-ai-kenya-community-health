"""
app.py - Main Streamlit Application for Kenya Health Agentic AI System
...
from autogen_agents.group_chat import run_group_chat, run_simple_query
...
def render_main_chat_interface():
    ...
    with col1:
        if st.button("📊 CHW Performance", use_container_width=True):
            user_query = "Show me overall CHW performance across all counties"
            send_button = True

    with col2:
        if st.button("🏥 County Comparison", use_container_width=True):
        ...
        if user_query.strip():
            with st.spinner("🧠 AI is analyzing your question..."):
                try:
                    ...
                    # Use the AI system to process the query
...
def run_predefined_query(query):
    ...
    with st.spinner(f"🧠 Running analysis: {query[:50]}..."):
        try:
            ...
            # Run the group chat with proper arguments
            response = run_group_chat(query, db_connection)
...
def main():
    ...
    try:
        ...
        with tab1:
            st.markdown("## 📊 Kenya Health System Overview")
