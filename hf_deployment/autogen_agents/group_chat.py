...
def get_model_client():
    """Configure model client for AutoGen 0.6+ with Groq priority"""

    # Priority 1: Groq via OpenAI-compatible API (Most Reliable)
    groq_api_key = os.getenv("GROQ_API_KEY")
    if groq_api_key:
        try:
            logging.info("🚀 Using Groq via OpenAI-compatible API")
    ...
    # Priority 2: Custom Hugging Face Client
    ...
    # Priority 3: Local Ollama (if available)
    try:
        import requests
    ...
    logging.warning("No model client available. Please set HF_API_KEY or OPENAI_API_KEY environment variable.")
    return None
...
async def call_agent_async(agent, message_content: str):
    ...
    try:
        ...
        logging.debug(f"Agent response content: {str(response)[:200]}...")
        return response
...
def call_agent_sync(agent, message_content: str):
...
def create_sql_generator_agent():
    """Create SQL generator agent using AutoGen 0.6+"""
...
def create_analysis_agent():
    """Create analysis agent using AutoGen 0.6+"""
    model_client = get_model_client()
    if not model_client:
        return None
...
def create_visualization_agent():
    """Create visualization agent using AutoGen 0.6+"""
    model_client = get_model_client()
    if not model_client:
        return None
