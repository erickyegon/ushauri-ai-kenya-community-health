...
class SmartFallbackClient(ChatCompletionClient):
    ...
    def _initialize_clients(self):
        """Initialize primary and fallback clients with strategic assignment"""
        try:
            # Primary: Hugging Face (more stable for system operations)
            self.primary_client = create_hf_client()
            if self.primary_client:
                logging.info("✅ Primary client (Hugging Face) initialized successfully")
            else:
                logging.warning("⚠️ Primary client (Hugging Face) not available")

            # Fallback: Groq (faster for specific operations when available)
            self.fallback_client = create_groq_client()
            if self.fallback_client:
                logging.info("✅ Fallback client (Groq) initialized successfully")
            else:
                logging.warning("⚠️ Fallback client (Groq) not available")

            # Set current client - prioritize stability
