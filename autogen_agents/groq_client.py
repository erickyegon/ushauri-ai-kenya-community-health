...
class GroqChatCompletionClient(ChatCompletionClient):
    ...
    def _is_rate_limited(self) -> bool:
        ...
        return current_time < _groq_rate_limit_tracker['cooldown_until']

    def _handle_rate_limit(self, error_msg: str):
        """Handle rate limiting by updating tracker and setting cooldown"""
        current_time = time.time()
        _groq_rate_limit_tracker['last_rate_limit'] = current_time
        _groq_rate_limit_tracker['rate_limit_count'] += 1

        # Progressive cooldown: start with 60 seconds, increase with each rate limit
        cooldown_seconds = min(60 * _groq_rate_limit_tracker['rate_limit_count'], 300)  # Max 5 minutes
        _groq_rate_limit_tracker['cooldown_until'] = current_time + cooldown_seconds

        logging.warning(f"Groq rate limited. Cooldown for {cooldown_seconds} seconds. Count: {_groq_rate_limit_tracker['rate_limit_count']}")
    ...
    async def create(
        ...
        **kwargs
        ...
        """Create a chat completion using Groq API with intelligent rate limit handling"""

        # Check if we're in cooldown period
        if self._is_rate_limited():
            cooldown_remaining = _groq_rate_limit_tracker['cooldown_until'] - time.time()
            logging.warning(f"Groq in cooldown for {cooldown_remaining:.1f} more seconds. Triggering fallback.")
            raise Exception("Groq API rate limited - in cooldown period")
        ...
        try:
            ...
            # Reset rate limit count on successful call
            if _groq_rate_limit_tracker['rate_limit_count'] > 0:
                logging.info("Groq API call successful - resetting rate limit counter")
                _groq_rate_limit_tracker['rate_limit_count'] = max(0, _groq_rate_limit_tracker['rate_limit_count'] - 1)
            ...
            if self._is_rate_limit_error(e):
                ...
                # Re-raise the exception so the fallback system can catch it
                raise Exception(f"Groq API rate limited: {e}")

            # For other errors, also raise to trigger fallback
